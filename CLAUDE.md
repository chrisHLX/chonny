...
**New cosmetic issue surfaced by this fix, not yet addressed:** now that Evoker's talent-derived kit actually renders, spells picked via the murlok import display with their raw `"(desc=Color)"` suffix still attached (e.g. the Spells table shows "Pyre (desc=Red)", not "Pyre") — the suffix is part of `spells.name` itself, used deliberately for internal matching (murlok comparison, duplicate disambiguation) throughout this codebase, so it can't just be stripped from the column. This was invisible before today (the spells weren't showing at all) and is now a real, visible papercut across most of Evoker's UI. Not fixed this session — would need a display-only accessor/transform in `<x-spells.table>` (or a `Spell::displayName` accessor) that strips the suffix at render time without touching the underlying matching logic anywhere else.
