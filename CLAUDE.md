...
### Heads up: the "current" patch string is intentionally frozen at `12.0.7.68453` — bumping it forks every patch-scoped table
...
**Practical consequence:** run `php artisan import:spelldata wow 12.0.7.68453` — not a real current WoW patch version, and not `12.0.7.68887` (that string was mistakenly re-derived from this file's own prose during a 2026-08-16 session and forked a stray, disconnected patch row before being caught and deleted — see `feedback_patch-version-bump-creates-new-row.md` in memory for the full incident). Before running `import:spelldata`, check the argument against `Patch::where('is_current', true)->first()->build_version` in the actual DB rather than any version string in this file's prose. A genuine patch-version transition is a larger workflow with its own steps (see `wow:patch-update`) — worth doing deliberately, not inferring from documentation.
