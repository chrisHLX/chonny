...
**Not yet done, deliberately** — no real manifest exists in this repo yet. This dev sandbox has no Blizzard credentials configured, so nothing here could generate real icon data; `wow:apply-icon-manifest` was verified against a throwaway fixture manifest (written and torn down in the same session, never committed) and a fixture-backed test (`tests/Feature/ApplyIconManifestTest.php`, using `--path` to avoid touching the real file). **Whoever has the real credentials** (or the already-fetched spell-icons — per this session's user, both the live server and their own dev PC already have them) needs to run `fetch-spell-icons.php` and `fetch-class-spec-icons.php` once there, then commit `data/spelldata/icon-manifest.json` + the three `storage/app/public/*-icons/` directories.
