...
## Description resolver gaps: SP Coefficient, sibling effect recovery, Variables-block leakage — found and fixed 2026-08-02

Surfaced while cross-checking Penance's in-game tooltip (via the addon's `resolved_description` export) against `ModuleSpellReferenceService::resolveDescription()`'s output for the same spell — the site version was visibly worse: raw unresolved template junk where the real tooltip showed either a real number or a clean "scales with X" gate. Four separate, stacked gaps, all in `resolveDescription()`'s token-resolution passes plus one upstream parser bug. **Locked files respected — zero changes to `QuizRunner.php`/`AiService.php`.**

**Gap 1 — `sN` tokens pointing at Spell Power scaling had no data to resolve to.** SimC's own effect-detail block carries `SP Coefficient: 0.5` (and separately `PvP Coefficient: ...`) on many damage/healing effects — confirmed as a real, first-class field in SimC's own `spelleffect_data_t` struct (verified via SimC's public GitHub source, not guessed), not something invented for this project. The parser was never capturing it, so any `$sN` token resolving to an SP-scaled effect fell straight to `(varies)` even when a real, known percentage was sitting right there in the source file. Fixed: `SpellDataFileParser` now captures `SP Coefficient:`/`PvP Coefficient:` per effect into new `spell_effects.sp_coefficient`/`pvp_coefficient` columns (migration `2026_08_02_000004`); `resolveDescription()`'s Pass 3 now tries `coefficientDisplay()` (`"≈50% of Spell Power"`) before falling back to `(varies)`, for both same-spell `sN` and cross-spell `<id>sN` token shapes.

**Gap 2 — effect-index tokens pointing at a sibling spell_id's effects, not the spell's own.** The Angelic Bulwark/Roar of Sacrifice pattern (documented above under "systemic effect-splitting"): a spell inherits its description text via `$@spelldesc<id>` from another spell_id, but its `$sN` tokens are meant to resolve against *that same other spell's* effect list, not its own (which is often empty or partial by design — SimC splits one logical ability across multiple internal spell_id rows). `resolveValueToken()`'s `sN` handling used a flat `$spell->effects->firstWhere('effect_index', $index)` with no fallback, so any spell missing that specific effect index silently produced `(varies)` even when a sibling row plainly had it. Fixed via new `findEffectByIndex()`: tries the spell's own effect first, and only if that effect is missing or has no resolvable value, searches same-name/same-patch sibling spells for the same `effect_index` with a real value. Quantified before shipping: **694 of 1,015** (68%) of spells hitting this pattern have a sibling with the needed effect index recoverable this way; the remaining 32% genuinely have no sibling data anywhere in the dataset and still correctly fall back to `(varies)`.

**Gap 3 — the `Variables:` block was never parsed as its own field, so it leaked into `description` as raw trailing text.** SimC's dump format has a distinct `Variables:` section (separate from `Description:`) defining named formula variables like `$penancedamage` used later in the description via `$<penancedamage>` tokens — e.g. Penance's real healing/damage number is gated behind Power of the Dark Side / Twilight Equilibrium / Castigation / Harsh Discipline talent modifiers, expressed as a `Variables:` formula referencing all four by spell_id. Because the parser had no dedicated handler for `Variables:`, its continuation lines were being swept up by the generic Description continuation-catch-all, appending raw SimC formula syntax straight into the description users would see. Fixed: new `variables` column on `spells` (migration `2026_08_02_000004`, `Spell::$fillable`), a dedicated `Variables:` line handler + continuation state (`$inVariablesContinuation`) mirroring the existing Description continuation handling, and all the other field handlers (Tooltip/Affecting Spells/Charges/Cooldown/Duration/Category/Effects/Description) now reset that flag on entry so they can't be mistaken for a Variables continuation.

**Gap 4 (bundled) — no display path existed for a resolved Variables reference at all.** Once Variables text was captured separately (Gap 3) instead of leaking into description, `resolveDescription()` still had nothing to *do* with a bare `$<penancedamage>`-style token — Pass 2/Pass 3's regexes only matched `$sN`/`${...}`/`$?a<id>[...]`/`$?c<n>[...]` shapes, not angle-bracket variable references, so the raw broken token passed straight through to the user. Fixed two ways: a new Pass 1.5 replaces any bare `$<[a-zA-Z0-9]+>` with `(varies)` (marking the description `uncertain`) as a safety net, and new `variablesModifiers(Spell $spell): Collection` extracts every spell_id referenced inside the `Variables:` block's own `$?a<id>[` / `$?s<id>[` conditional gates, resolves them to real spell names, and renders as `"Scales with: Power of the Dark Side, Twilight Equilibrium, Castigation, Harsh Discipline"` under the description in `<x-spells.table>` — a correctly-scoped "here's what affects this number" instead of either silence or a fabricated single value. Deliberately does not attempt to evaluate the Variables formula arithmetic itself (would require modeling conditional talent-state combinations) — the "flag, don't guess" posture holds here same as everywhere else in this pipeline.

**Re-imported and re-verified end-to-end** (same `migrate:fresh` + re-import + re-seed cycle as every other schema-affecting fix in this document): Penance's description now shows its real base description text plus `Scales with: Power of the Dark Side, Twilight Equilibrium, Castigation, Harsh Discipline` instead of raw `$<penancedamage>` junk; Angelic Bulwark and Roar of Sacrifice both resolve real absorb/heal percentages via sibling recovery instead of `(varies)`; spot-checked several more SP-scaled heal/damage spells across Discipline Priest's kit for correct `"≈N% of Spell Power"` display. Full test suite + `ModuleSpellReferenceServiceTalentGatingTest` confirmed zero regressions beyond the same 12 pre-existing, unrelated failures already documented elsewhere in this file.

## `${...}` arithmetic fabricated a `0` for any unresolved embedded token — fixed 2026-09-06

Found surveying every spec's rendered kit for description junk: Rogue **Eviscerate** displayed *"1 point : 0 damage 2 points: 0 damage 3 points: 0 damage ..."*, Druid **Rip** *"1 point : 0 over 1.3 sec"*, **Maim** *"1 point : 0 damage"*, DH **Spirit Bomb** *"absorbing 0 damage plus 0 for each Soul Fragment"*, Warlock **Unstable Affliction** *"If dispelled, deals 0 damage"* — plus a long tail of *"you suffer 0% of damage prevented"* / *"reduced by 0%"* / *"up to 0% Haste"* / *"stacking -1 times"* / *"jumping to -1 additional nearby enemies"*.

**Root cause — `ModuleSpellReferenceService::resolveDescriptionUncached()` Pass 2 (`${...}` arithmetic).** When an embedded value token couldn't be resolved, Pass 2 substituted the literal string `'0'` and evaluated the expression anyway: `${$m1*3}` → `${0*3}` → `0` → *"3 points: 0 damage"*. This is the exact "confidently wrong number, worse than an honest placeholder" that `effectValue()`'s own docblock and Pass 3 already guard against for the bare-`$sN` case — Pass 2 just never got the same treatment. `$uncertain` was already being set, but the visible text still read `0`.

**Three coordinated fixes, resolver-only — no schema change, no re-import:**

1. **Unresolved token → discard the whole `${...}` to `(varies)`, but only from a poisoning position.** A `0` substitution is only correct where `0` is the arithmetic identity for the token's position: an *additive term* (`+$s3`, `$d-$s1`) or a *dividend* (`$s2/100`). As a *multiplicand* (`$m1*3`) it annihilates the product; as a *divisor* (`$s1/$s2`) it breaks the expression. Pass 2 now inspects the operators flanking each null token (`PREG_OFFSET_CAPTURE`) — `prevOp`/`nextOp` of `*`, or `prevOp` of `/` → poisoned → `(varies)`; anything else keeps `0`. This preserves **Alter Time** (`${$110909d+$s3}` → real *"10 seconds"*, `$s3` is a nonexistent "extension talent" term) and **Nature's Guardian** (`${$Xs1*(1+$s2/100)}` → real base *"40%"*, `$s2` is a 0-when-untalented percent modifier) while still nuking Eviscerate's `${$m1*N}`.

2. **`$mN` / `$MN` tokens now resolve — against the spell's OWN effect only, never `findEffectByIndex()` sibling recovery.** `resolveValueToken()` previously handled `$sN`/`$d`/`$<id>sN` but not `$mN` ("min value" of effect N), so charge-grant talents fell straight through: **Prosperity** *"Swiftmend now has ${$m1+1} charges"* (own effect #1 `Base Value: 1`) rendered `(varies)` instead of *"2 charges"*. Same for **Ice Ward**, **Focused Thunder**, **Pool of Mists**. Sibling recovery is deliberately excluded here: Eviscerate's own effect #1 is `0/0` (real per-combo-point damage is AP-coefficient-scaled, uncaptured), but a same-name sibling carries an unrelated `35` (its Energy *cost*) — pulling that in via `findEffectByIndex()` rendered a confident, wrong *"35 damage per point"*. Own-effect-only keeps the safe win and lets `effectValue()`'s `0/0` guard return null so fix #1 falls back to `(varies)`.

3. `$mN`/`$MN` cross-spell forms (`$<id>mN`) fall through to null as before — not worth the ambiguity for a token shape that's rare and, where it does occur, usually a poisoning-position multiplicand anyway.

**Blast radius, measured before/after against a class-only build context over all 2,671 `${...}`-bearing spells** (`git stash`-diffed the resolver): **493 rendered descriptions changed**, **0 genuine "real number → (varies)" regressions**. Of the 493: ~352 fabricated `0`/`-1`/`-2`/`-3` → an honest `(varies)` or a real value; ~141 poisoned `${...}` (Eviscerate family) → `(varies)`; **128 number-morphs where own-effect `$mN` turned a fake `0` into the real value** — *"You gain 0 Rage"* → *"25 Rage"* (Bear Form), *"reduces cooldown by 0 sec"* → *"5 sec"* (Demonic Circle CD), *"deal 200% ... instead of the usual 200%"* → *"250% ..."* (Elemental Fury), *"0% more often"* → *"25% more often"*, etc. In the 40 precomputed spec kits, entries showing a fabricated `0` in a damage/heal/absorb phrase went **21 → 0**; `uncertain=true` entries **1,997 → 1,887**.

**Deliberately NOT fixed this pass** (pre-existing, dataset-wide, `game-data.md` already documents the first as intentionally-deferred): the literal `.1` suffix some source formulas carry (`${$m1/-1000}.1 sec` — SimC's decimal-place hint, dumped literally; now that the leading value is real instead of `0` this reads as e.g. *"2.1 sec"* / *"0.5.1 sec"* on ~8 Hunter Sentinel / Explosive Shot talents), negative-modifier values shown with a literal `-` (*"slowing movement speed by -50%"*, *"every -1.1 sec"*), the `$leffect:effects;` / `$l...:...;` pluralization token, `$?c<n>` condition codes, and the malformed `$?a137008][...]` Pass-1 conditional shape on DK Reaper's Mark. Each is its own separate token-family gap.

**Tests:** 5 new cases in `tests/Feature/Services/ModuleSpellReferenceServiceDescriptionTest.php` (multiplicand → `(varies)`, additive term keeps computed value, dividend inside `(1+$x/100)` treated as 0, `$mN` resolves against own effect, divisor → `(varies)`). Full suite: same 12 pre-existing, unrelated failures, 0 new. Spell cache version bumped; `wow:precompute-spell-kits` re-run for all 40 specs.

## `$<varname>` Variables formulas + `${...}.N` precision suffix + a pointer-target Variables backfill — fixed 2026-09-06

Reported via **Shield Discipline** (`spell_id 47755`) rendering *"restore **(varies)%** of your maximum mana"* on the Discipline Priest spell surface, and showing no icon. Two unrelated root causes.

**The `(varies)%`** — 47755's description is inherited via `$@spelldesc197045`, and the real number lives in **197045's** `Variables: $mana=${$47755s1/100}.1`. Three coordinated fixes:

1. **`ImportSpellData::resolveDescriptionReferences()` now backfills the pointer target's `variables` too**, not just its `description` text — but only when the pointing spell has no `variables` of its own (never clobbers). The inherited description's `$<varname>` tokens resolve against the *target's* Variables block, so without this 47755 had the text but no formula. Plain `upsertTrack()` column write, same in-place-re-import-safe shape as the `description` backfill it sits next to.

2. **`ModuleSpellReferenceService` Pass 1.5 now evaluates a `$<varname>` when it can.** Blanket-evaluating Variables formulas is still out (Penance's `$castigation=$?a193134[${1}][${0}]` chains conditional talent branches — see `variablesModifiers()`). New `parseVariableDefs()` returns `[]` for any block containing `$?` anywhere; otherwise it maps `$name` → right-hand side. If the referenced var is defined as **one bare `${...}` expression** (optional `.N` suffix), Pass 1.5 inlines that expression and lets Pass 2 evaluate it under its own "unresolved token → `(varies)`" guards. Everything else still falls back to `(varies)`. Penance verified unchanged.

3. **Pass 2 now consumes a trailing `.N` immediately after `${...}`** — SimC's decimal-precision specifier (**690 occurrences dataset-wide**, effectively always `.1`), previously passed through as literal text (*"lasts 3.1 sec"* where the real value is 3; *"0.5.1%"* once a `$<var>` resolved to 0.5). Dropped; `formatNumber()` already renders sensible precision. This also cleans up the `${…/-1000}.1 sec` artifacts flagged (deliberately deferred) in the fabricated-`0` entry above — Reduced Blink GCD now reads *"by 0.5 sec"*, Demonic Circle CD *"by 5 sec"*.

Verified: Shield Discipline → *"restore 0.5% of your maximum mana"* (`$47755s1`=50 → `50/100`), `uncertain=false`. Tests: 3 new cases in `ModuleSpellReferenceServiceDescriptionTest.php` (unconditional `$<var>` evaluates, `$?`-containing block stays `(varies)`, `.N` suffix consumed). **In-place re-import** (`variables`/`description` are plain columns, no unique-key component — same as the split-by-tree note above, no `migrate:fresh`).

**The missing icon** — 47755 is `source=baseline, spec_id=NULL`, not passive-flagged, not a talent, no cooldown: it was in **none** of `fetch-spell-icons.php`'s target-set buckets (talent_node_entries / pvp_talents / verified_override / explicit-baseline-cooldown / rotation-steps / rotation-mechanics / cc-chains). It only reaches the page because `preferSelectedPerName()` collapses it against its hidden sibling `197045` (which does have an icon). Rather than add an 8th narrow source query — the exact recurring gap every dated comment in that block already describes — `fetch-spell-icons.php` now also reads **every `spells.id` in `data/spell-kits/*/*.json`** (`entries[].spellId`), the definitive "everything SpellExplorer/WowComps actually render" set written by `wow:precompute-spell-kits`. Any spell that shows on a page is in a kit file by construction, so this subsumes the piecemeal sources for gap-detection purposes. Across all 40 rendered kits, only 6 spells had a null icon before this (Shield Discipline, Master's Call, Healthstone, Demonic Healthstone, Seduction, Fatebound Coin (Tails)). Depends on `wow:precompute-spell-kits` having been run for current data first (a normal deploy / `wow:refresh-match-derived` does that).
...
relationship labels, missing magnitude conversions) are defects to close, not settled
limitations — "flag, don't guess" means *don't fabricate a number*, and the resolution is to
get the real data, not to leave a placeholder on screen.
