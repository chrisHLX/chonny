...
This section exists because it didn't, until a 2026-08-26 refresh (pulling a genuinely newer WoW build's SimC dumps — 12.1.0 Live — into the one existing patch row — at the time under a frozen version string; since 2026-09-16 the row is relabelled in place instead, see CLAUDE.md's "The patch row is relabelled in place") surfaced a real bug that a purely aggregate reading of the importer's own console summary made look like normal churn. Two things are worth separating clearly: what a refresh actually *does*, and how to *know* it went well — the second didn't exist anywhere before this incident.
...
3. `php artisan import:spelldata wow` — no patch argument. It imports into the current patch row and relabels it to the build the SimC headers report (since 2026-09-16; before that the argument had to be a frozen string, because any new one forked a new row).
