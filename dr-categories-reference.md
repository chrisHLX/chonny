...
- **Cyclone — 5s, confirmed 2026-08-13.** Matches its own raw `Duration: 5 seconds` field for once — but that's the *stored* value being right, not a sign this field is reliable in general (see Incapacitating Roar directly below, and Kidney Shot/Gouge above, where the stored value is wrong). `pvp_duration_seconds` set.
