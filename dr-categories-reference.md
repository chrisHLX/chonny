...
**Second source checked and rejected (2026-08-13): [Maxroll — Crowd Control Diminishing Returns](https://maxroll.gg/wow/resources/crowd-control-diminishing-returns).** Looked more promising on the surface than the Wowhead source above — explicitly dated "Current Patch: 12.0.7, Last Updated: June 17, 2026," with a full per-spell PvE/PvP duration table across 8 categories (adds Displacements and Taunts as their own categories, splits Disarms out). The domain expert reviewed it directly and found it **not reliable**, including for content it claims is current: no PvP CC in the current game lasts 8 seconds anymore (this source's table lists several at 8s — Freezing Trap, Polymorph, Sap, Hex, Shackle Undead, Mind Control, Blind), and Demon Hunter's listed stun duration is wrong. The Roots table read as broadly correct on inspection (Entangling Roots checked out), with one flagged exception (Wild Charge — listed as 4s, domain expert suspects 2-3s) — so "checked and rejected" applies to the source as a whole, not a signal that every individual row is wrong; it's that the source can't be trusted as-is and each row would need the same one-at-a-time verification as everything else in this file, which defeats the point of using it as a shortcut. Also caught independently: this source's own reset-timer figure (16s) is already a patch behind the 20s this file has confirmed current for 12.1 — even the parts that aren't outright wrong are stale. Its 6s-cap and Maim=5s numbers happened to match what's independently confirmed above; its Cyclone=6s does not (this file's confirmed 5s, from both the domain expert directly and this project's own raw spell data, stands — the source is the one assumed wrong here, not the other way around). Conclusion: do not use this or any similar aggregator site as a bulk-import source for this file — the existing one-spell-at-a-time domain-expert-confirmation process is the only reliable path, and no faster external shortcut has been found despite two sources checked now.
...
**Confidence correction (2026-08-13):** the domain expert cited throughout this file re-reviewed it directly and flagged that it reads as more settled than it actually is — they got at least one duration wrong themselves during the original session, and the 7→5 category consolidation noted below wasn't necessarily a deliberate decision. Treat every number/category in this file as provisional until re-confirmed, not as closed. The two updates just below are the first re-confirmation pass.

- **Maim — 5s, confirmed 2026-08-13.** Not the 6s cap; a real exception below it, same shape as Gouge. `pvp_duration_seconds` set.
- **Cyclone — 5s, confirmed 2026-08-13** (directly by the domain expert, and independently matches this project's own raw spell data's `Duration: 5 seconds` field). A secondary source (Maxroll, see above) claims 6s — checked and dismissed; see that section for why the source itself, not this number, is what's untrusted here. Matching the stored raw value here is coincidence, not a sign that field is reliable in general — see Incapacitating Roar directly below, and Kidney Shot/Gouge above, where the same raw field is confirmed wrong. `pvp_duration_seconds` set.
