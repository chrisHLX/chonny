...
test('selecting a spec and length renders the real Peak Burst Example for that length', function () {
    ...
    // topDpsWindowsByLength.6.damage. Updated 2026-09-05 after the archive's oldest-500-match
    // cull + a fresh 300-match pull + all-spec-rotations.php regeneration legitimately moved the
    // real number again — re-check this value directly from the committed JSON any time the
    // rotation data is regenerated, don't assume it's stable across reruns.
    expect($html)->toContain('1,137,390');
});
