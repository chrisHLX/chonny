...
@else
    <div class="flex flex-wrap gap-2 mb-8">
        @foreach($subjects as $subject)
            <a href="{{ route_with_context(request()->route()->getName(), ['subject_id' => $subject->id]) }}"
               class="px-4 py-2 rounded-full text-sm font-medium transition
                      {{ $subject->id == $subjectId ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }}">
                {{ $subject->name }}
            </a>
        @endforeach
    </div>
