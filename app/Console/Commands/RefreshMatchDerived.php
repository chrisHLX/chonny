 ...
 *   7. wow:analyze-cc-targeting — writes data/arena-logs/cc-targeting.json: measured, per ability,
 *      whether real players use each crowd-control ability on the enemy healer or their damage
 *      dealers, normalised against what an ability with no preference would hit. Read by step 8.
 *      Scans raw logs directly (the only step here besides step 2 that does), so it is the second
 *      slowest — roughly 20s for the whole archive.
 *   8. wow:build-burst-guides — writes data/claudes-guides/burst-guides/{class}/{spec}.json (the
 *      Burst Guides page's own source). Added 2026-09-04 alongside that page, same "don't let a
 *      new match pull silently leave this stale" discipline that motivated this whole
 *      orchestrator. Since 2026-09-06 it aggregates the archive's full per-window corpus
 *      ({archive}/rotations/{class}/{spec}.jsonl, regenerated by step 2) rather than reading the
 *      single promoted window from step 3 — see App\Http\Services\BurstGuideBuilder. Pure local
 *      JSON computation (no raw-log decompression), still fast. It also PRUNES guide files for
 *      specs that no longer have promoted rotation data, the same way promoteRotations() prunes
 *      the rotations themselves at step 3 — without that, a spec whose matches were culled kept
 *      serving a guide built from deleted matches.
 *   9. TalentSelectionService::bumpSpellCacheVersion() — every step above changes what a cached
...
class RefreshMatchDerived extends Command
{
    protected $signature = 'wow:refresh-match-derived
        ...
        {--skip-burst-guides : Skip step 8 (wow:build-burst-guides)}';
    ...
    public function handle(TalentSelectionService $talentService): int
    {
        ...
        $this->step('1/8 — Crowd Control chain corpus (wow:find-cc-chains --json)', fn () => $this->option('skip-cc-chains')
            ? $this->skipped()
            : $this->callArtisan('wow:find-cc-chains', ['--json']));

        if (! $this->option('skip-rotations')) {
            $ok = $this->step('2/8 — Regenerate burst-window rotations (all-spec-rotations.php)', function () use ($scriptsDir) {
            ...
            if ($ok) {
                $this->step('3/8 — Promote regenerated rotations into the live repo', fn () => $this->promoteRotations($archivePath));
                $this->step('4/8 — Enrich promoted windows with real talent builds', fn () => $this->callArtisan('wow:enrich-rotation-talents'));
                $this->step('5/8 — Enrich promoted windows with real champion/target mechanics', fn () => $this->callArtisan('wow:enrich-rotation-mechanics'));
            } else {
                $this->warn('  Skipping promotion + enrichment — rotation regeneration itself failed.');
            }
        } else {
            $this->step('2-5/8 — Burst-window regeneration + enrichment', fn () => $this->skipped());
        }

        $this->step('6/8 — Class Guide playstyle data (wow:analyze-spec-playstyle, every spec)', fn () => $this->option('skip-playstyle')
            ? $this->skipped()
            : $this->refreshPlaystyle());

        $this->step('7/8 — Crowd-control targeting (wow:analyze-cc-targeting)', fn () => $this->callArtisan('wow:analyze-cc-targeting'));

        $this->step('8/8 — Burst Guides (wow:build-burst-guides)', fn () => $this->option('skip-burst-guides')
            ? $this->skipped()
    ...
    private function promoteRotations(string $archivePath): bool
    {
        ...
        // Promotion used to be copy-only, so a spec that STOPPED having an export kept its last
        // promoted file forever. Found 2026-09-06: after the 2026-09-05 archive cull removed the
        // early-expansion matches, four specs (Blood DK, Guardian Druid, Augmentation Evoker,
        // Protection Warrior) had no matches left and produced no export — and the site went on
        // serving their 2026-08-31 burst windows, built from matches that had been deliberately
        // deleted for being unrepresentative, with nothing indicating the data was three weeks
        // stale. Copying forward is only half of "promote"; removing what is no longer produced
        // is the other half, and its absence is silent by construction.
        $removed = 0;
        foreach (File::glob(base_path('data/arena-logs/rotations/*/*.json')) as $live) {
            if (! isset($promoted[$live])) {
                File::delete($live);
                $removed++;
                $this->warn('  Removed '.basename(dirname($live)).'/'.basename($live).' — no longer produced by the archive (that spec has no matches on file).');
            }
        }

        if ($removed > 0) {
            $this->line("  Removed {$removed} stale rotation file(s).");
        }
