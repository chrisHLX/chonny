...
class ArenaLogService
{
    ...
    public function searchLatestMatches(string $bracket, int $offset, int $count = 50): array
    {
        $query = '
            query($bracket: String, $count: Int, $offset: Int) {
              latestMatches(
                ...
                combats {
                  __typename
                  ... on ArenaMatchDataStub { id playerTeamRating durationInSeconds startTime }
                }
              }
            }
        ';
        ...
        foreach ($combats as $combat) {
            ...
            $results[] = [
                ...
                // Unix milliseconds, same field/units as metadata/{id}.json's own 'startTime'
                // (confirmed via a real stored match, 2026-09-05) — null if the API ever omits
                // it for a given entry, never assumed present.
                'startTime' => isset($combat['startTime']) ? (int) $combat['startTime'] : null,
            ];
        }
    ...
    /** Upper bound on a burst guide's shown step count — see buildBurstGuideSequence()'s docblock. */
    private const BURST_GUIDE_MAX_STEPS = 16;

    /**
     * Builds the "Burst Guide" sequence for one spec — a definite, filtered, ordered series of
     * real ability presses distilled from a real archived burst window (added 2026-09-04, direct
     * design brief: "a Definite series or combinations of keys (GCDs) to press"). Distinct from
     * both `rotationForSpec()`'s raw `topDpsWindow*` (every real cast, unfiltered — what the
     * "Real Observed Window" sequence section already shows) and
     * `offensiveDefensiveClassification()` on its own (a static per-spell CD list with no
     * ordering/sequence at all, what WoW Comps' Offensive/Defensive tabs show) — this combines
     * the two: real observed ORDER, filtered down to only the presses that matter for a burst.
     *
     * SOURCE WINDOW: the longest available `topDpsWindowsByLength` entry that actually has steps
     * (30s, falling back to 20/12/10/6s) — deliberately NOT the 12s window the rest of the site
     * defaults to. A short window rarely contains enough data for the repeat-detection step below
     * to ever find a genuine repeat; confirmed empirically before this was built, using
     * Assassination Rogue's own real data: 4 of 5 available lengths (6/10/12/20s) never produced
     * a clean repeat at all, only the 30s window did — cutting a "definite series" guide down to
     * whatever a 12s window happens to contain would make the repeat-truncation rule below almost
     * never actually fire.
     *
     * FILTER (per step, "does pressing this result in a real damage increase — or is it Crowd
     * Control on the kill target"): a step is DROPPED only when its spell is classified via
     * `offensiveDefensiveClassification()` as defensive-only (defensive=true, offensive=false)
     * AND does not carry a `dr_category` (Crowd Control always survives this filter regardless of
     * classification — "Target Priority: if it's part of the burst on the kill target, keep it").
     * Everything else is KEPT, including every spell `offensiveDefensiveClassification()` has no
     * opinion on at all (base rotation builders/finishers/interrupts/racials — that classifier
     * only ever covers CDs with a real offensive-or-defensive purpose, so an unclassified spell
     * is presumed to be an ordinary rotational press, not noise). Confirmed against real data
     * before shipping: Rogue's own window never actually contains Evasion (the illustrative
     * example from the design brief), but Shadow Priest's real window does contain Fade — a
     * genuine defensive-only classified spell woven into a real burst window — and this rule
     * correctly drops it while keeping Power Infusion/Voidform/Void Torrent (offensive) and
     * Psychic Scream (Crowd Control, no classification needed) from the same window.
     *
     * KNOWN LIMITATION, flagged rather than solved: "CC only if it's ON THE KILL TARGET" can't
     * currently be verified per step. `offensive-rotations.php` (the separate wow-arena-archive
     * project)'s raw `SPELL_CAST_SUCCESS` regex extraction does not capture each individual
     * cast's own destination GUID, only the window's overall tracked (damage) target — so a CC
     * step appearing inside a kill-target-scoped window is PRESUMED, not proven, to have been
     * cast on that same target (e.g. a Blind used mid-window to peel the enemy healer, while
     * continuing to damage the actual kill target, would currently still be kept). Closing this
     * for real needs that script's log-line regex extended to capture destGUID per cast — out of
     * this feature's scope; recorded here per this project's "flag, don't guess" discipline.
     *
     * TRUNCATION ("once we start repeating ability sequences we can stop"): once the filtered
     * sequence contains an immediately-adjacent block of 2+ abilities that repeats back-to-back
     * (e.g. Mutilate, Mutilate, Envenom, Mutilate, Mutilate, Envenom), the guide stops right after
     * that block's SECOND occurrence — see truncateAtRepeat() below. A single ability repeating
     * twice in a row (period 1 — an ordinary double-tap of one builder before a finisher) does
     * NOT count as "the sequence repeating" on its own; only a multi-step block looping does. If
     * no such block is ever found within the observed window, the whole filtered sequence is
     * shown, capped at self::BURST_GUIDE_MAX_STEPS as a sane upper bound on a "simple layout".
     *
     * Deliberately spell_id-only in what it computes — resolving each id to a real Spell
     * (name/cooldown/dr_category/icon) is left to the caller, same "never freeze what will go
     * stale" discipline as every other spell reference in data/claudes-guides/ (see that folder's
     * README). This method only decides WHICH ids, in WHAT order.
     *
     * @return array{sourceLengthSeconds:int,sourceMatchId:?string,spellIds:array<int,int>,rawStepCount:int,filteredStepCount:int,truncatedAtRepeat:bool}|null
     */
    public function buildBurstGuideSequence(string $classSlug, string $specSlug): ?array
    {
        $rotation = $this->rotationForSpec($classSlug, $specSlug);
        if (!$rotation || empty($rotation['topDpsWindowsByLength'])) {
            return null;
        }

        $window = null;
        $length = null;
        foreach ([30, 20, 12, 10, 6] as $candidate) {
            $w = $rotation['topDpsWindowsByLength'][$candidate] ?? null;
            if ($w && !empty($w['steps'])) {
                $window = $w;
                $length = $candidate;
                break;
        ...
        $rawSteps = $window['steps'];

        $stepIds = collect($rawSteps)->pluck('spellId')->filter()->unique();
        $patchId = \App\Models\Patch::where('is_current', true)->value('id');
        $spellsById = $stepIds->isEmpty()
            ? collect()
            : \App\Models\Spell::whereIn('spell_id', $stepIds)->where('patch_id', $patchId)->get()->keyBy('spell_id');

        $classification = $this->offensiveDefensiveClassification();

        $filtered = [];
        foreach ($rawSteps as $step) {
            $spell = $spellsById[$step['spellId']] ?? null;
            $isCc = $spell?->dr_category !== null;

            if ($isCc) {
                $filtered[] = $step['spellId'];
                continue;
            }

            $cls = $classification['bySpellId'][$step['spellId']] ?? ($classification['byName'][$step['name']] ?? null);
            $isPureDefensive = $cls && $cls['defensive'] && !$cls['offensive'];
            if ($isPureDefensive) {
                continue;
            }
        ...
        $truncated = $this->truncateAtRepeat($filtered);
        $capped = array_slice($truncated, 0, self::BURST_GUIDE_MAX_STEPS);

        return [
            'sourceLengthSeconds' => $length,
            'sourceMatchId' => $window['matchId'] ?? null,
            'spellIds' => $capped,
            'rawStepCount' => count($rawSteps),
            'filteredStepCount' => count($filtered),
            'truncatedAtRepeat' => count($truncated) < count($filtered) || count($capped) < count($truncated),
        ];
    }

    /**
     * Finds the earliest point an adjacent block of 2+ items repeats back-to-back (e.g.
     * [A,B,C,A,B,C,...] truncates right after the 6th item, showing the block exactly twice) and
     * truncates there. A lone item repeating twice in a row (period 1) is deliberately NOT
     * treated as a repeat — see buildBurstGuideSequence()'s own docblock for why. Returns the
     * input unchanged if no such block exists anywhere in it.
     */
    private function truncateAtRepeat(array $ids): array
    {
        $n = count($ids);
        for ($end = 4; $end <= $n; $end++) {
            for ($p = 2; $p <= intdiv($end, 2); $p++) {
                if (array_slice($ids, $end - 2 * $p, $p) === array_slice($ids, $end - $p, $p)) {
                    return array_slice($ids, 0, $end);
                }
            }
        }
    ...
    /**
     * The real, full 3-person roster of the team OPPOSITE $healerName in $matchId — i.e. "the
     * comp that landed the CC" on that healer, not the healer's own team. Built 2026-09-06 for
     * Top 10 CC Chains, closing the gap flagged in that page's own docblock: `casters` was
     * previously derived only from the chain's own steps (whoever happened to cast a recorded
     * CC ability), which could show fewer than 3 real teammates when only part of a team
     * contributed a CC step. This resolves the REAL roster from the match's own metadata
     * instead, via `reaction` — confirmed live (2026-09-06) that `reaction` cleanly splits a
     * match's units into exactly two groups (matching `matchRosterHasSpec()`'s existing
     * external_spec_id lookup convention, and the same `reaction` field
     * `analyzeKillCausally()`'s roster already carries).
     *
     * $healerName is used ONLY to look up their own unit internally, to find which `reaction`
     * value is "the other side" — never returned or exposed by this method, matching this
     * project's standing no-real-player-name-on-any-public-page discipline.
     *
     * Returns up to 3 {classSlug, specSlug} pairs (fewer if the match's own metadata doesn't
     * have a full recorded roster for that side — a real, if rare, data-completeness gap, not
     * guessed or padded). Empty array if the healer's own unit can't be found in the metadata at
     * all (e.g. a stale/renamed player) or the metadata file is missing.
     *
     * $specsByExternalId — an optional pre-fetched `Specialization::with('gameClass')->get()->
     * keyBy('external_spec_id')` map. Callers resolving many chains in a loop (e.g. TopCcChains)
     * should build this once and pass it in, rather than re-querying the whole (small but
     * non-trivial) specs table on every single call.
     *
    ...
    public function resolveOpposingTeamSpecs(string $matchId, string $healerName, ?\Illuminate\Support\Collection $specsByExternalId = null): array
    {
        ...
        $healerReaction = null;
        foreach ($units as $u) {
            if (($u['name'] ?? null) === $healerName && str_starts_with($u['id'] ?? '', 'Player-')) {
                $healerReaction = $u['reaction'] ?? null;
                break;
            }
        }

        if ($healerReaction === null) {
            return [];
        }
        ...
        foreach ($units as $u) {
            ...
            if (($u['reaction'] ?? null) === $healerReaction) {
                continue; // same side as the healer — not the comp that landed the CC
            }
